...
class Model(nn.Module):
    ...
    def predict(
        ...
        **kwargs,
        ...
        #如果预测其predictor未初始化，则根据args参数进行初始化
    ...
    def train(
        ...
        **kwargs,
        ...
        #检查是否为pytorch模型
        self._check_is_pytorch_model()
        #如果当前会话(self.session)已经加载了模型
        if hasattr(self.session, "model") and self.session.model.id:  # Ultralytics HUB session with loaded model
            #检查是否有额外的训练参数被加入，有则警告，将会忽略本地训练参数
            if any(kwargs):
        ...
        #从kwargs中获取配置文件路径（cfg）,如果没有则使用默认的 self.overrides
        overrides = yaml_load(checks.check_yaml(kwargs["cfg"])) if kwargs.get("cfg") else self.overrides
        #创建一个custom字典，其中包含数据相关的默认配置
        custom = {"data": DEFAULT_CFG_DICT["data"] or TASK2DATA[self.task]}  # method defaults
        #将overrides、custom、kwargs以及一个"mode": "train"的键值对合并到args字典中，其中kwargs中的参数具有最高优先级
        args = {**overrides, **custom, **kwargs, "mode": "train"}  # highest priority args on the right
        #如果指定了resume，则将resume路径设置为self.ckpt_path
        if args.get("resume"):
            args["resume"] = self.ckpt_path

        #使用trainer或通过self._smart_load("trainer")加载的训练器来初始化self.trainer，并传入args和回调函数
        self.trainer = (trainer or self._smart_load("trainer"))(overrides=args, _callbacks=self.callbacks)
        #如果不是从断点恢复训练（not args.get("resume")），则手动设置模型
        if not args.get("resume"):  # manually set model only if not resuming
            #加载模型和模型配置文件
            self.trainer.model = self.trainer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml)
            self.model = self.trainer.model

            #如果启用了HUB功能（SETTINGS["hub"]为真）且当前没有会话（self.session为空），则尝试在HUB中创建模型。如果成功，则使用HUB会话的create_model方法创建模型，并检查模型是否成功创建（通过检查self.session.model.id）
